                ...
                <div class="d-flex gap-3">
                    ...
                    <a href="{{url('create-quote')}}">
                        <button class="import-leads-button">
                            ...
                            <span class="button-text">{{ __('app.quotes.create-title') }}</span>
            ...
            <div class="card-container">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row g-4">
                            <div class="col-12 col-md-4">
                                ...
                                <input type="text" class="form-control" id="field1" placeholder="Change your T&C from here" value="{{$config->terms ?? ''}}" name="terms" >
                            </div>
                            <div class="col-12 col-md-4">
                                <label for="field2" class="form-label">Quote Logo</label>
                                <input type="file" class="form-control" id="field2" placeholder="Pipeline" name="quote_logo">
                            </div>

                            <div class="col-12 col-md-4">
                              <?php if($config && $config->quote_logo != null){ 
                                ?> 
                                 <img src="{{ asset('uploads/'.$config->quote_logo) }}" width="222px" height="118px" alt="" style="object-fit: cover;">
                ...
                <div class="card card-default">
                    <div class="card-body">
                        ...
                        <div class="row g-4">
                            <form id="bulk-delete-form" method="POST" action="{{ url('delete-selected-quotes') }}">
                            @csrf
                            <div class="table-responsive">
                                <table class="table new-table data-table-export" data-export-title="Quotes" data-export-filename="Quotes">
                                    ...
                                    ?>
                                                <tr class="odd gradeX">
                                                    <td><input type="checkbox" name="selected_quotes[]" value="{{ $quote->id }}"></td>
                                                    <td class="">{{$quote->subject}} </td>
                                                    <td class=""><a href="{{url('users?id='.$quote->owner)}}">{{$owner_name}}</a></td>
                                                    <td class="">{{$quote->expired_at}} </td>
                                                    <td class=""><a href="{{url('persons?id='.$quote->person)}}">{{$person_name}}</a></td>
                                                    <td class="">${{number_format($sub_total)}}</td>
                                                    <td class="">${{number_format($quote->discount_total_amount, 2)}}</td>
                                                    <td class="">${{number_format($quote->tax_total_amount, 2)}}</td>
                                                    <td class="">${{number_format($quote->order_total_input, 2)}}</td>
                                                    <td class="">{{$quote->expired_at}} </td>
                                                    <td class="">{{$quote->created_at}} </td>
                                 
                                                    <td class="action-icons d-flex gx-3">
                                                        <a href="{{ url('delete-quote/'.$quote->id) }}" class="delete-link-confirm">
...
<script>
    ...
    document.getElementById('select-all').addEventListener('click', function(event) {
        let checkboxes = document.querySelectorAll('input[name="selected_quotes[]"]');
...
<div class="offcanvas offcanvas-end" tabindex="-1" id="offFilter" aria-labelledby="offFiltertLabel">
    ...
    <div class="row">
        ...
        <div class="col-md-12 mb-3">
            <label for="owner" class="form-label">Sales Owner</label>
            <select class="form-control" name="owner" >
                <option value="">Select Owner</option>
                @foreach($owners as $owner)
                    <option value="{{ $owner->user_id }}" {{ request('owner') == $owner->user_id ? 'selected' : '' }}>
                        {{ $owner->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="col-md-6 mb-3">
            <label for="expire_start_date" class="form-label">Expire Start</label>
            <input type="date" class="form-control" name="expire_start_date" value="{{ request('expire_start_date') }}">
        </div>

        <div class="col-md-6 mb-3">
            <label for="expire_end_date" class="form-label">Expire End</label>
            <input type="date" class="form-control" name="expire_end_date" value="{{ request('expire_end_date') }}">
        </div>
    ...
    <a href="{{ url('quotes') }}" class="btn btn-danger">Clear</a>
</form>

...
@section('content')
                                        ...
                                        <div class="row g-4">
                                            ...
                                            <div class="col-12 col-md-4">
                                                <div class="d-flex align-items-center mb-2">
                                                    <label for="field4" class="form-label me-2">Emails</label>
                                                    <button class="btn add-more-button p-0" onclick="addEmailField()"
                                                        type="button">
                                                        <!-- SVG icon here -->
                                                        <span>{{ __('app.common.add_more') }}</span>
                                                    </button>

                                                </div>
                                                <div>
                                                    <input type="email" class="form-control" placeholder="Emails"
                                            ...
                                            {{-- <div class="col-12 col-md-4 contact-numbers-container">
                                                        ...
                                                        <div class="d-flex align-items-center mt-2">
                                                            <div class="form-check form-check-inline">
                                                                <input class="form-check-input" type="radio"
                                                                    ...
                                                                    value="work">
                                                                <label class="form-check-label"
                                                                    for="inlineRadio1">{{ __('app.common.work') }}</label>
                                                            </div>
                                                            <div class="form-check form-check-inline">
                                                                <input class="form-check-input" type="radio"
                                                                    name="number_types[0]" id="number-home-0"
                                                                    value="home">
                                                                <label class="form-check-label"
                                                                    for="inlineRadio2">{{ __('app.common.home') }}</label>
                                                            </div>
    ...
    <script>
        ...
        function addEmailField() {
            ...
            const original = container.querySelector(".single-email-field");

            // Clone the original input section
            const clone = original.cloneNode(true);
            const input = clone.querySelector("input");

            // Clear the input value in the clone
            input.value = "";

            // Check if trash icon already exists in the clone and remove it (just in case)
            const existingTrash = clone.querySelector(".fa-trash");
            if (existingTrash) existingTrash.remove();

            // Create a new trash icon
            const trash = document.createElement("i");
            trash.className = "fa-solid fa-trash ms-2 text-danger";
            trash.style.cursor = "pointer";
            trash.onclick = function() {
                clone.remove();
            };

            // Append trash icon to the clone
            clone.appendChild(trash);

            // Append the updated clone to the container

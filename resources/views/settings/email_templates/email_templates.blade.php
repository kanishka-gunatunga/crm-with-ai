...
<div class="main-scrollable">
    <div class="page-container">
        <div class="page-title-container">
            <div class="d-flex justify-content-between">
                <h3 class="page-title">
                    {{ __('app.settings.email-templates.title') }}
                </h3>
                <div class="d-flex gap-3">


                    <a href="{{url('create-email-template')}}">
                        <button class="import-leads-button">
                            ...
                            <span class="button-text">{{ __('app.settings.email-templates.create-title') }}</span>
                        </button>
                    </a>
                </div>
                ...
                <div class="card card-default">
                    <div class="card-body">
                        <div class="d-flex gap-3 mb-4 justify-content-between align-items-center">
                            <div>
                                <h3 class="card-title">{{ __('app.settings.email-templates.title') }}</h3>
                            </div>
                            <div>
                        ...
                        <div class="row g-4">
                             <form id="bulk-delete-form" method="POST" action="{{ url('delete-selected-templates') }}">
                            @csrf
                            <div class="table-responsive">
                                <table class="table new-table data-table-export" data-export-title="Email Templates" data-export-filename="Email Templates">
                                   ...
                                   ?>
                                        <tr class="odd gradeX">
                                            <td><input type="checkbox" name="selected_templates[]" value="{{ $template->id }}"></td>
                                            <td class="">{{$template->id}} </td>
                                            <td class="">{{$template->name}} </td>
                                            <td class="">{{$template->subject}} </td>
                                            <td class="action-icons d-flex gx-3">
                                                <a href="{{ url('delete-email-template/'.$template->id) }}" class="delete-link-confirm">
                                                ...
                                                <a href="{{ url('edit-email-template/'.$template->id) }}">
                                                <div class="text-muted" type="button">
                                                    <svg width="20" height="20" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
 ...
 <button type="submit" class="btn btn-danger btn-sm mb-2 delete-form-confirm" >Delete Selected</button>
    ...
    document.getElementById('select-all').addEventListener('click', function(event) {
        let checkboxes = document.querySelectorAll('input[name="selected_templates[]"]');
...
<div class="offcanvas offcanvas-end" tabindex="-1" id="offFilter" aria-labelledby="offFiltertLabel">
    ...
    <div class="row">
        <div class="col-md-12 mb-3">
            <label for="sku" class="form-label">{{ __('app.datagrid.id') }}</label>
            <input type="text" class="form-control" name="id" value="{{ request('id') }}">
        </div>
        <div class="col-md-12 mb-3">
            <label for="name" class="form-label">{{ __('app.datagrid.name') }}</label>
            <input type="text" class="form-control" name="name" value="{{ request('name') }}">
        </div>
        <div class="col-md-12 mb-3">
            <label for="name" class="form-label">{{ __('app.datagrid.subject') }}</label>
    ...
    <a href="{{ url('email-templates') }}" class="btn btn-danger">Clear</a>
</form>

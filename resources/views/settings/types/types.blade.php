...
<div class="main-scrollable">
    <div class="page-container">
        <div class="page-title-container">
            <div class="d-flex justify-content-between">
                <h3 class="page-title">
                    {{ __('app.settings.types.title') }}
                </h3>
                <div class="d-flex gap-3">


                    <a href="{{ url('create-type') }}">
                        <button class="create-btn">
                            ...
                            <span class="button-text">{{ __('app.settings.types.create-title') }}</span>
                ...
                <div class="card card-default">
                    <div class="card-body">
                        <div class="d-flex gap-3 mb-4 justify-content-between align-items-center">
                            <div>
                                <h3 class="card-title">{{ __('app.settings.types.title') }}</h3>
                            </div>
                            <div>
                        ...
                        <div class="row g-4">
                             <form id="bulk-delete-form" method="POST" action="{{ url('delete-selected-types') }}">
                            @csrf
                            <div class="table-responsive">
                                <table class="table new-table data-table-export" data-export-title="Types" data-export-filename="Types">
                                   ...
                                   ?>
                                        <tr class="odd gradeX">
                                            <td><input type="checkbox" name="selected_types[]" value="{{ $type->id }}"></td>
                                            <td class="">{{$type->id}} </td>
                                            <td class="">{{$type->name}} </td>
                                            <td class="action-icons d-flex gx-3">
                                                <a href="{{ url('delete-type/'.$type->id) }}" class="delete-link-confirm">
 ...
 <script>
    ...
    document.getElementById('select-all').addEventListener('click', function(event) {
        let checkboxes = document.querySelectorAll('input[name="selected_types[]"]');

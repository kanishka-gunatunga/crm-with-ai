...
@section('content')
        ...
        <div class="page-container">
            <div class="page-title-container">
                <div class="d-flex justify-content-between">
                    <h3 class="page-title">
                        {{ __('app.settings.pipelines.title') }}
                    </h3>
                    <div class="d-flex gap-3">


                        <a href="{{ url('create-pipeline') }}">
                            <button class="create-btn">
                                ...
                                <span class="button-text">{{ __('app.settings.pipelines.create-title') }}</span>
                    ...
                    <div class="card card-default">
                        <div class="card-body">
                            <div class="d-flex gap-3 mb-4 justify-content-between align-items-center">
                                <div>
                                    <h3 class="card-title">{{ __('app.settings.pipelines.title') }}</h3>
                                </div>
                                <div>

                                    @if (in_array(strtolower('export-pipelines'), array_map('strtolower', $permissions)))
                            ...
                            <div class="row g-4">
                                <form id="bulk-delete-form" method="POST" action="{{ url('delete-selected-pipelines') }}">
                                    @csrf
                                    <div class="table-responsive">
                                        <table class="table new-table data-table-export" data-export-title="Pipelines"
                                            data-export-filename="Pipelines">
                                            ...
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th>{{ __('app.datagrid.rotten-days') }}</th>
                                                    <th>{{ __('app.datagrid.is-default') }}</th>
                                   ...
                                   ?>
                                                <tr class="odd gradeX">
                                                    <td><input type="checkbox" name="selected_pipelines[]"
                                                            value="{{ $pipeline->id }}"></td>
                                                    <td class="">{{ $pipeline->id }} </td>
                                                    <td class="">{{ $pipeline->name }} </td>
                                                    <td class="">{{ $pipeline->rotting_days }} </td>
                                                    <td class="">{{ $pipeline->is_default }} </td>

                                                    <td class="action-icons d-flex gx-3">

                                                        <a href="{{ url('delete-pipeline/' . $pipeline->id) }}"
                                                        ...
                                                        <a href="{{ url('edit-pipeline/' . $pipeline->id) }}">
                                                            <div class="text-muted" type="button">
                                                                <svg width="20" height="20" viewBox="0 0 18 18"
                                                                    fill="none" xmlns="http://www.w3.org/2000/svg">
                                                                    <rect width="18" height="18" rx="2.90323"
    ...
    <script>
        ...
        document.getElementById('select-all').addEventListener('click', function(event) {
            let checkboxes = document.querySelectorAll('input[name="selected_pipelines[]"]');

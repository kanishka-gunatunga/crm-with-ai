                ...
                <div class="d-flex gap-3">
                    ...
                    <a href="{{url('create-role')}}">
                        <button class="import-leads-button">
                            ...
                            <span class="button-text">{{ __('app.settings.roles.create-title') }}</span>
                ...
                <div class="card card-default">
                    <div class="card-body">
                        <div class="d-flex gap-3 mb-4 justify-content-between align-items-center">
                            <div>
                                <h3 class="card-title"> {{ __('app.settings.roles.title') }}</h3>
                            </div>
                            <div>
                        ...
                        <div class="row g-4">
                            <form id="bulk-delete-form" method="POST" action="{{ url('delete-selected-roles') }}">
                            @csrf
                            <div class="table-responsive">
                                <table class="table new-table data-table-export" data-export-title="Roles" data-export-filename="Roles">
                                   ...
                                   ?>
                                        <tr class="odd gradeX">
                                            <td><input type="checkbox" name="selected_roles[]" value="{{ $role->id }}"></td>
                                            <td class="">{{$role->id}} </td>
                                            ...
                                            <td class="">{{$role->permission_type}}</td>
                                             ...
                                             <td class="action-icons d-flex gx-3">
                                                <a href="{{ url('delete-role/'.$role->id) }}" class="delete-link-confirm">
...
<script>
    ...
    document.getElementById('select-all').addEventListener('click', function(event) {
        let checkboxes = document.querySelectorAll('input[name="selected_roles[]"]');
...
<div class="offcanvas offcanvas-end" tabindex="-1" id="offFilter" aria-labelledby="offFiltertLabel">
    ...
    <div class="row">
        ...
        <div class="col-md-12 mb-3">
            <label for="person" class="form-label">Permission Type</label>
            <select class="form-control" name="permission_type" >
                <option value="">Select</option>
                    <option value="all" {{ request('permission_type') == 'all' ? 'selected' : '' }}>
                        All
                    </option>
                    <option value="custom" {{ request('permission_type') == 'custom' ? 'selected' : '' }}>
                        Custom
                    </option>
            </select>
        </div>
    ...
    <a href="{{ url('roles') }}" class="btn btn-danger">Clear</a>
</form>

    ...
    <main class="login-container">
        @if ($config && !$config->logo == null)
            <div class="text-center mb-5">
                <img src="{{ asset('uploads/' . $config->logo) }}" alt="Logo" class="signuplogo"
        ...
        <div class="login-card card card-default" style="background:{{ $form->form_background_color ?? '#ffffff' }}">
            ...
            @endif
        {{-- <pre>{{ print_r($personNameField, true) }}</pre>
        <pre>{{ print_r($lead_attributes, true) }}</pre> --}}
            <form action="{{ url('/web-form-submit/' . $form->uid) }}" method="POST" data-parsley-validate>
                ...
                <div class="form-fields">
                    ...
                    @if ($personNameField)
                        <div class="mb-3">
                            <label for="username" class="form-label"
                                style="color:{{ $form->lable_color ?? '#546e7a' }} !important">
                                {{ $personNameField['label'] ?? 'Name' }}
                            </label>
                            <input type="text" class="form-control" id="username"
                                placeholder="{{ $personNameField['placeholder'] ?? '' }}" name="person_name"
                    ...
                    @if ($personEmailField)
                        <div class="mb-3">
                            ...
                            <input type="email" class="form-control" id="username"
                                placeholder="{{ $personEmailField['placeholder'] ?? '' }}" name="person_email"
                    ...
                    @if ($personPhoneField)
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="username"
                                placeholder="{{ $personPhoneField['placeholder'] ?? '' }}" name="person_phone"
                    ...
                    @if ($personOrganizationField)
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="person_organization" aria-label="Default select example"
                                {{ !empty($personOrganizationField['required']) ? 'required' : '' }}>
                                <option selected disabled hidden>{{ $personOrganizationField['placeholder'] ?? '' }}
                                </option>
                    ...
                    @if ($form->create_lead_enabled == 'on')
                        ...
                        @if ($leadTitleField)
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="username"
                                    placeholder="{{ $leadTitleField['placeholder'] ?? '' }}" name="lead_title"
                        ...
                        @if ($leadValueField)
                            <div class="mb-3">
                                ...
                                <input type="number" step="any" class="form-control" id="username"
                                    placeholder="{{ $leadValueField['placeholder'] ?? '' }}" name="lead_value"
                        ...
                        @if ($leadDescriptionField)
                            <div class="mb-3">
                                ...
                                <textarea style="height:unset;" class="form-control" id="exampleFormControlTextarea1" name="lead_description"
                                    rows="3" placeholder="{{ $leadDescriptionField['placeholder'] ?? '' }}"
                                    {{ !empty($leadDescriptionField['required']) ? 'required' : '' }}></textarea>
                        ...
                        @if ($leadSourceField)
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="lead_source" aria-label="Default select example"
                                    {{ !empty($leadSourceField['required']) ? 'required' : '' }}>
                                    <option selected disabled hidden>{{ $leadSourceField['placeholder'] ?? '' }}
                                    </option>
                        ...
                        @if ($leadTypeField)
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="lead_type" aria-label="Default select example"
                                    {{ !empty($leadTypeField['required']) ? 'required' : '' }}>
                        ...
                        @if ($leadOwnerField)
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="lead_owner" aria-label="Default select example"
                                    {{ !empty($leadOwnerField['required']) ? 'required' : '' }}>
                                    <option selected disabled hidden>{{ $leadOwnerField['placeholder'] ?? '' }}
                                    </option>
                ...
                <div>
                    <button type="submit" class="login-btn w-100 btn btn-info"
                        style="width: 100%;background:{{ $form->submit_btn_color ?? '#0e90d9' }}">{{ $form->button_lable ?? 'Submit' }}</button>
                </div>

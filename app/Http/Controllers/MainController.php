...
class MainController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        if ($request->isMethod('get')) {
            ...
            $all_organizations = Organization::orderBY('id', 'DESC')->get();
            $this_month_organizations = $all_organizations->filter(function ($organization) {
                return $organization->created_at->month === Carbon::now()->month &&
                    $organization->created_at->year === Carbon::now()->year;
            });
            ...
            $all_quotations = Quote::orderBY('id', 'DESC')->get()->where('owner', $userId);
            $this_month_quotations = $all_quotations->filter(function ($quotation) {
            ...
            // dd($recent_quotations->first()->leadData->title);
            ...
            $my_activities = LeadActivity::whereIn('lead_id', $leadIds)->take(5)->orderBy('id', 'DESC')->get();

            // $my_activities = LeadActivity::orderBy('id', 'DESC')->get();
            ...
            return view('dashboard', [
                ...
                'this_month_organizations' => $this_month_organizations,
    ...
    public function get_events($date)
    {
        ...
        $authUser = Auth::user();
        $leadIds = Lead::where('sales_owner', $authUser->id)->pluck('id');
        $events = LeadActivity::whereIn('lead_id', $leadIds)->get()->sortByDesc('id');
